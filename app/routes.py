...
def save_picture(form_picture):
    # Changing name file of the user profile before upload it
    random_hex = secrets.token_hex(8)
    _, f_ext = os.path.splitext(form_picture.filename)
    picture_fn = random_hex + f_ext
    picture_path = os.path.join(app.root_path, 'static/profile_pics', picture_fn)

    # Re-size user profile picture
    output_size = (125, 125)  # Size assigned
    i = Image.open(form_picture)  # Open picture
    i.thumbnail(output_size)  # Cut picture
    i.save(picture_path)  # Save picture
